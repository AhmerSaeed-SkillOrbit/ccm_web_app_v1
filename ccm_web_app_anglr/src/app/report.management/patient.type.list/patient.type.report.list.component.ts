...
@Component({
    ...
    moduleId: module.id,
    templateUrl: 'patient.type.report.list.component.html',
    // styleUrls: ['../ccm.plan.component.css']
    providers: [
        DatePipe // this pipe is used to change date format
    ],
})
export class PatientTypeReportListComponent implements OnInit {
    files: any;
    ...
    ngOnInit(): void {
        ...
        if (!this.isLogin) {
            ...
            // this.listPagePermission = this._utilityService.checkUserPermission(this.user, 'view_report_patient_type');
            this.listPagePermission = this._utilityService.checkUserPermissionViewPermissionObj(this.userPermissions, 'view_report_patient_type');
    ...
    onPatientTypeSelect() {
        const patientType = this.patientTypeList.filter(pt => pt.id == +this.patientTypeId);
        ...
        this.patientTypeId = patientType[0].id;
        this.patientType = patientType[0];
    ...
    loadPatientTypeList() {
        ...
        this._setupService.getPatientTypeList().subscribe(
            (res) => {
                ...
                this.patientTypeList = ptList;

                // console.log('patientTypeList', this.patientTypeList);
        ...
        if (this.listPagePermission) {
            ...
            this._reportService.getPatientTypeReportListCount(this.doctorId, this.patientTypeId, this.startDate, this.endDate, this.searchKeyword).subscribe(
                (res) => {
                    ...
                    this._reportService.getPatientTypeReportListPagination(this.pageIndex, this.pageSize, this.doctorId, this.patientTypeId, this.startDate, this.endDate, this.searchKeyword).subscribe(
                        (res) => {
    ...
    generatePDF() {
        ...
        this.reportListAll.forEach((element, index) => {
            let data = {
                ...
                "Patient Type": element.patientType.name || "NA",
